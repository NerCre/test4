...
(() => {
  ...
  function initOnePage() {
    ...
    // --- Pictograms: 緊急入力（ワンページ） ---
    function setBtnIcon(btnId, src) {
      const btn = document.getElementById(btnId);
      if (!btn) return;
      if (btn.querySelector('img.btn-icon')) return;
      const label = (btn.textContent || '').trim();
      btn.classList.add('with-icon');
      btn.innerHTML = `<img class="btn-icon" src="${src}" alt=""><span class="btn-text">${label}</span>`;
    }

    function applyPictogramsOnePage() {
      const segMap = {
        segConsciousOne: { yes: 'pict_conscious_yes.png', no: 'pict_conscious_no.png', unknown: 'pict_conscious_unknown.png' },
        segBreathingOne: { yes: 'pict_breath_yes.png', no: 'pict_breath_no.png', unknown: 'pict_unknown_q.png' },
        segBleedingOne: { yes: 'pict_bleed_yes.png', no: 'pict_bleed_no.png', unknown: 'pict_unknown_q.png' },
        segPainOne: { yes: 'pict_pain_yes.png', no: 'pict_pain_no.png', unknown: 'pict_unknown_q.png' },
      };

      Object.keys(segMap).forEach((segId) => {
        const wrap = document.getElementById(segId);
        if (!wrap) return;
        const map = segMap[segId];
        wrap.querySelectorAll('button.seg-btn').forEach((btn) => {
          const val = btn.dataset.val;
          const src = map[val];
          if (!src) return;
          if (btn.classList.contains('has-picto')) return;
          const label = (btn.textContent || '').trim();
          btn.classList.add('has-picto');
          btn.innerHTML = `<img class="seg-icon" src="${src}" alt="${label}"><span class="seg-text">${label}</span>`;
        });
      });

      // 場所
      setBtnIcon('btnScanLocationOne', 'pict_location_qr.png');
      setBtnIcon('btnLocationPickMapOne', 'pict_location_qr.png');
      setBtnIcon('btnLocationSetManualOne', 'pict_location_qr.png');
      setBtnIcon('btnLocationUnknownOne', 'pict_unknown_q.png');

      // 被災者
      setBtnIcon('btnScanVictimOne', 'pict_victim_qr.png');
      setBtnIcon('btnSearchVictimOne', 'pict_victim_qr.png');
    }
    ...
    // 事故区分定義（ユーザー指定の順序・内容）
    const accidentDefs = [
      { key: 'fall', label: '転落', img: 'pict_acc_fall.png' },
      { key: 'crush', label: '挟まれ', img: 'pict_acc_crush.png' },
      { key: 'flying', label: '飛来', img: 'pict_acc_flying.png' },
      { key: 'collapse', label: '倒壊', img: 'pict_acc_collapse.png' },
      { key: 'burn', label: '熱傷', img: 'pict_acc_burn.png' },
      { key: 'hazard', label: '有害物', img: 'pict_acc_hazard.png' },
      { key: 'electric', label: '感電', img: 'pict_acc_electric.png' },
      { key: 'collision', label: '激突', img: 'pict_acc_collision.png' },
      { key: 'explosion', label: '爆発', img: 'pict_acc_explosion.png' },
      { key: 'other', label: 'その他', img: 'pict_acc_other.png' },
    ];
    ...
    if (iconWrap && iconWrap.children.length === 0) {
      accidentDefs.forEach((def) => {
        ...
        div.innerHTML = `<div class="icon"><img src="${def.img}" alt="${def.label}"></div><div class="label">${def.label}</div>`;
  ...
  function wireGlobalEvents() {
    ...
    if (aedBack)
      aedBack.addEventListener('click', () => {
        ...
        if (ov) ov.classList.remove('hidden');
        if (detail) detail.classList.add('hidden');
        // Invoke callback if present; always close map view first
    ...
    if (stretcherBack)
      stretcherBack.addEventListener('click', () => {
        ...
        if (typeof cbSt === 'function') {
          setTimeout(() => {
            try { cbSt(); } catch (e) { console.error(e); }
          }, 0);
        }
      });
