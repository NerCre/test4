...
(() => {
  ...
  function startEmergencyInitial() {
    // Step 1: Prompt user to confirm location and call ambulance
    // 最初の誘導ポップアップ。
    // 説明文を簡潔にし、場所選択せずに進むオプションと戻るボタンを追加します。
    // 最初の誘導ポップアップ。場所の特定方法を案内し、
    // その後の選択肢では救急車呼び出し・SMS送信・入力続行のみを提示します。
    showOverlay(
      'すぐに場所を確認して救急車を呼んでください。',
      [
        {
          label: '場所QRを読む',
          style: 'primary',
          onClick: () => {
            ...
            // 場所を確定した後の選択肢を提示
            showOverlay(
              '入力が完了したら、救急車を呼ぶか入力を続けるか選択してください。',
              [
                {
                  label: '救急車を呼ぶ',
                  style: 'emergency',
                ...
                },
                {
                  ...
                  onClick: () => {
                    closeOverlay();
                    const smsBtn = document.getElementById('btnSendSms');
      ...
      [
        {
          ...
          style: 'primary',
          onClick: () => {
            // オーバーレイを閉じ、SMS送信後に必要な物品の案内を行うよう
  ...
  const nav = {
    ...
    restartAll() {
      nav.stack = ['view-home'];
      resetFlow();
      // ワンページ緊急入力（view-onepage）の入力状態もリセットする
      try {
        resetOnePageInputs();
      } catch (e) {
        console.warn('resetOnePageInputs failed', e);
      }
      nav.show('view-home', { push: false });
    },
  };
  ...
  /**
   * view-onepage（緊急入力）の入力データとUIをリセットする
   * - stateOne（__stateOneRef）
   * - ボタン選択状態（active/danger）
   * - ラベル・入力欄・事故区分の選択
   */
  function resetOnePageInputs() {
    // 進行中のオーバーレイは閉じる
    try { closeOverlay(); } catch {}

    // SMS送信後コールバックを解除
    try { window.__afterSmsAction = null; } catch {}
    ...
    // UI: 手入力欄
    const locManual = document.getElementById('locationManualOne');
    ...
    const iconWrap = document.getElementById('accidentIcons');
    if (iconWrap) {
      iconWrap.querySelectorAll('.icon-item').forEach((el) => el.classList.remove('active'));
    }

    // UI: 救急車ボタンの見た目を更新（常に押せる仕様）
    try {
